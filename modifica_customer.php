        ...
        <body>
            ...
            <div class="col-md-12">
                <form method="post">
                    <?php
                        if(empty($_POST)){
                            $customer = $pdo->query("SELECT * FROM $_GET[tabla] WHERE id = $_GET[id]")->fetch(PDO::FETCH_ASSOC);
                        }else{
                            $customer = $_POST;
                        }
                    ?>
                    <div class="row">
                       ...
                       <div class="col-md-8">
                           <input class="alta" id="name" type="text" placeholder="Nombre" name="name" value="<?php echo $customer['name']?>">
                       </div>
                        <div class="col-md-3">
                            <label class="des_alta" for="fname">Inserta apellido:</label>
                        </div>
                        <div class="col-md-8">
                            <input class="alta" id="last_name" type="text" placeholder="Apellido" name="last_name" value="<?php echo $customer['last_name']?>">
                        </div>
                        <div class="col-md-3">
                            <label class="des_alta" for="fname">Inserta telefono:</label>
                        </div>
                        <div class="col-md-8">
                            <input class="alta" id="phone" type="text" placeholder="Telefono" name="phone" value="<?php echo $customer['phone']?>">
                        </div>
                        <div class="col-md-3">
                            <label class="des_alta" for="fname">Inserta direccion:</label>
                        </div>
                        <div class="col-md-8">
                            <input class="alta" id="address" type="text" placeholder="Direccion" name="address" value="<?php echo $customer['address']?>">
                        </div>
                        <div class="col-md-3">
                                ...
                                <?php
                                    foreach ($users as $user){
                                        if($customer['users_id'] === $user['id']){
                                            echo '<option  selected="selected" value="'.$user['id'].'">'.$user['id'].' '.$user['email'].'</option>';
                                        }else{
